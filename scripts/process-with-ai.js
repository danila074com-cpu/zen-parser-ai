// scripts/process-with-ai.js — версия с 8000 токенами
...
console.log("🚀 Запускаем AI-обработку статей (8000 токенов)...");
    ...
    for (let i = 2; i <= Math.min(worksheet.rowCount, maxArticles + 1); i++) {
      ...
      try {
        ...
        const titleResponse = await openai.chat.completions.create({
          ...
          messages: [{
            role: "user",
            content: `Создай новый эмоциональный заголовок в стиле "MadeSimple" для Яндекс Дзен. 
Оригинальный заголовок: "${originalTitle}"
Верди ТОЛЬКО готовый заголовок без комментариев.`
          }],
          temperature: 0.8,
          max_tokens: 100
        });
        ...
        // 2. Генерация текста - УВЕЛИЧЕН ДО 8000 ТОКЕНОВ!
        console.log("   ✍️ Переписываем текст (8000 токенов)...");
        const textResponse = await openai.chat.completions.create({
          model: AI_MODEL,
          messages: [{
            role: "user", 
            content: `Ты — профессиональный автор в стиле "MadeSimple" для Яндекс Дзен.
            
ПЕРЕПИШИ ЭТОТ ТЕКСТ ПОДРОБНО, сохранив ВЕСЬ сюжет, эмоции и диалоги.

ТРЕБОВАНИЯ:
- Сохрани всех персонажей и ключевые события
- Добавь внутренние монологи, эмоции, чувства
- Опиши детали обстановки, атмосферу, настроение
- Сделай диалоги живыми и естественными
- ОБЪЕМ ДОЛЖЕН БЫТЬ НЕ МЕНЕЕ 3000 СЛОВ
- Пиши в реалистичном, эмоциональном стиле Дзен

НЕ СОКРАЩАЙ сюжет! РАСШИРЬ описание сцен и переживаний персонажей.
...
Верди ТОЛЬКО готовый текст без комментариев.`
          }],
          temperature: 0.7,
          max_tokens: 8000  // 🔥 УВЕЛИЧЕНО ДО 8000!
        });
